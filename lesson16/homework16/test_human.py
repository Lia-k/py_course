...
def test_check_grow_if_human_is_dead(human, monkeypatch):
    ...
    monkeypatch.setattr(human, "_Human__is_alive", mock_alive)
    with pytest.raises(Exception, match=f"{human._Human__name} is already "
                                        f"dead..."):
        human.grow()
...
@pytest.mark.parametrize("change_gender,gender,expected",
                         [("female", "male", "male"),
                          ("male", "female", "female")],
                         ids=["change_to_male", "change_to_female"])
def test_check_change_gender(human,
                             change_gender,
                             gender,
                             expected,
                             monkeypatch):
...
@pytest.mark.parametrize("change_gender,gender",
                         [("female", "female"),
                          ("male", "male")],
                         ids=["change_female_to_female",
                              "change_male_to_male"])
def test_check_change_gender_if_gender_is_same(human,
                                               change_gender,
                                               gender,
                                               monkeypatch):
    """
    ...
    monkeypatch.setattr(human, "_Human__validate_gender", mock_validate_gender)
    monkeypatch.setattr(human, "_Human__gender", change_gender)
    with pytest.raises(Exception, match=f"{human._Human__name} already has "
                                        f"gender '{human._Human__gender}'"):
        human.change_gender(gender)
